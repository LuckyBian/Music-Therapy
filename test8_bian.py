...
def small():
    print("11111")
    global record_c_n, record_refer, record_c, record_x, record_y, record_pos
    while True:
        print("({},{})".format(X, Y))
        first_play = 0
    ...
    while True:
        ...
        for hand, handType in zip(handData, handsType):
            if handType == 'Right':
                ...
                for i in range(12):
                    for j in range(12):
                        if grid_w // 12 * j + (width - grid_w) // 2 < hand[8][0] < grid_w // 12 * (j + 1) + (
                                width - grid_w) // 2 and grid_h / 12 * i < hand[8][1] < grid_h / 12 * (i + 1):
                            cv2.putText(flip_img, '(' + str(horizontal[j]) + ',' + str(vertical[i]) + ')',
                                        (hand[8][0] + 10, hand[8][1] + 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 2)
                            Y = horizontal[i]
        ...
        # draw the grid
        h1 = cv2.line(flip_img, ((width - grid_w) // 2, grid_h // 12 * 0),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 0), (0, 255, 0), 2)
        h2 = cv2.line(h1, ((width - grid_w) // 2, grid_h // 12 * 1),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 1),
                      (0, 255, 0), 2)
        h3 = cv2.line(h2, ((width - grid_w) // 2, grid_h // 12 * 2),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 2),
                      (0, 255, 0), 2)
        h4 = cv2.line(h3, ((width - grid_w) // 2, grid_h // 12 * 3),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 3),
                      (0, 255, 0), 2)
        h5 = cv2.line(h4, ((width - grid_w) // 2, grid_h // 12 * 4),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 4),
                      (0, 255, 0), 2)
        h6 = cv2.line(h5, ((width - grid_w) // 2, grid_h // 12 * 5),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 5),
                      (0, 255, 0), 2)
        h7 = cv2.line(h6, ((width - grid_w) // 2, grid_h // 12 * 6),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 6),
                      (0, 255, 0), 2)
        h8 = cv2.line(h7, ((width - grid_w) // 2, grid_h // 12 * 7),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 7),
                      (0, 255, 0), 2)
        h9 = cv2.line(h8, ((width - grid_w) // 2, grid_h // 12 * 8),
                      ((width - grid_w) // 2 + grid_w, grid_h // 12 * 8),
                      (0, 255, 0), 2)
        h10 = cv2.line(h9, ((width - grid_w) // 2, grid_h // 12 * 9),
                       ((width - grid_w) // 2 + grid_w, grid_h // 12 * 9), (0, 255, 0), 2)
        h11 = cv2.line(h10, ((width - grid_w) // 2, grid_h // 12 * 10),
                       ((width - grid_w) // 2 + grid_w, grid_h // 12 * 10), (0, 255, 0), 2)
        h12 = cv2.line(h11, ((width - grid_w) // 2, grid_h // 12 * 11),
                       ((width - grid_w) // 2 + grid_w, grid_h // 12 * 11), (0, 255, 0), 2)
        h13 = cv2.line(h12, ((width - grid_w) // 2, grid_h // 12 * 12),
                       ((width - grid_w) // 2 + grid_w, grid_h // 12 * 12), (0, 255, 0), 2)

        v1 = cv2.line(h12, ((width - grid_w) // 2, 0), ((width - grid_w) // 2, grid_h), (0, 255, 0), 2)
        v2 = cv2.line(v1, ((width - grid_w) // 2 + grid_w // 12, 0), ((width - grid_w) // 2 + grid_w // 12, grid_h),
                      (0, 255, 0), 2)
        v3 = cv2.line(v2, ((width - grid_w) // 2 + grid_w // 12 * 2, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 2, grid_h), (0, 255, 0), 2)
        v4 = cv2.line(v3, ((width - grid_w) // 2 + grid_w // 12 * 3, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 3, grid_h), (0, 255, 0), 2)
        v5 = cv2.line(v4, ((width - grid_w) // 2 + grid_w // 12 * 4, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 4, grid_h), (0, 255, 0), 2)
        v6 = cv2.line(v5, ((width - grid_w) // 2 + grid_w // 12 * 5, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 5, grid_h), (0, 255, 0), 2)
        v7 = cv2.line(v6, ((width - grid_w) // 2 + grid_w // 12 * 6, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 6, grid_h), (0, 255, 0), 2)
        v8 = cv2.line(v7, ((width - grid_w) // 2 + grid_w // 12 * 7, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 7, grid_h), (0, 255, 0), 2)
        v9 = cv2.line(v8, ((width - grid_w) // 2 + grid_w // 12 * 8, 0),
                      ((width - grid_w) // 2 + grid_w // 12 * 8, grid_h), (0, 255, 0), 2)
        v10 = cv2.line(v9, ((width - grid_w) // 2 + grid_w // 12 * 9, 0),
                       ((width - grid_w) // 2 + grid_w // 12 * 9, grid_h), (0, 255, 0), 2)
        v11 = cv2.line(v10, ((width - grid_w) // 2 + grid_w // 12 * 10, 0),
                       ((width - grid_w) // 2 + grid_w // 12 * 10, grid_h), (0, 255, 0), 2)
        v12 = cv2.line(v11, ((width - grid_w) // 2 + grid_w // 12 * 11, 0),
                       ((width - grid_w) // 2 + grid_w // 12 * 11, grid_h), (0, 255, 0), 2)
        v13 = cv2.line(v12, ((width - grid_w) // 2 + grid_w // 12 * 12, 0),
                       ((width - grid_w) // 2 + grid_w // 12 * 12, grid_h), (0, 255, 0), 2)
...
t1 = threading.Thread(target=big)
