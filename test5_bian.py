...
fluidsynth.set_instrument(2, 50)
            ...
            for i in range(12):
                for j in range(12):
                    if grid_w//12*j+(width-grid_w)//2 < hand[8][0] < grid_w//12*(j+1)+(width-grid_w)//2 and grid_h/12*i < hand[8][1] < grid_h/12*(i+1):
                        ...
                        t = loop_coordinate(horizontal[j], horizontal[i])

                        print(numpy.array_equal(note,t))

                        if numpy.array_equal(note,t) == False:
                            s = True
                            T = threading.Thread(target=play, args=(t,))
                            note = t
                            T.start()
                            s = False
